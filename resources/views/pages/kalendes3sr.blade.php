...
@section('content')
    ...
    <body>
            ...
            <aside class="side-listAcara">
                ...
                <div class="flex justify-between items-center mb-4 gap-2">
                    ...
                    <div class="space-y-3 max-h-[400px] overflow-y-auto pr-2" id="activity-list-container">
                        @forelse($aktivitasBulanIni ?? [] as $activity)
                            <div class="bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-xl border-l-4 border-blue-500 hover:shadow-md transition">
                                <div class="flex justify-between items-start mb-2">
                                    <h4 class="font-bold text-gray-800">{{ $activity->title }}</h4>
                                    <span class="text-xs px-2 py-1 rounded-full {{ $activity->status === 'selesai' ? 'bg-green-200 text-green-800' : 'bg-yellow-200 text-yellow-800' }}">
                                        {{ ucfirst($activity->status) }}
                                    </span>
                                </div>
                                <p class="text-sm text-gray-600 mb-2">{{ $activity->desk }}</p>
                                <div class="flex items-center text-xs text-gray-500 space-x-3">
                                    <span><i data-lucide="calendar" class="w-3 h-3 inline"></i> {{ \Carbon\Carbon::parse($activity->date)->format('d M Y') }}</span>
                                    @if($activity->time)
                                        <span><i data-lucide="clock" class="w-3 h-3 inline"></i> {{ $activity->time }}</span>
                                    @endif
                                </div>
                            </div>
                        @empty
                            <div class="text-center py-8 text-gray-500">
                                ...
                                <p>Tidak ada acara untuk bulan ini</p>
                            </div>
                        @endforelse
                    </div>
                </div>
        ...
        <div id="modalAcara" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl p-6 max-w-md w-full mx-4">
                <h3 id="modalTitle" class="text-xl font-bold mb-4">Tambah Acara</h3>
                <form id="formAcara" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="inputTitle" class="w-full px-3 py-2 border rounded-lg" required>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2">Deskripsi</label>
                        <textarea name="desk" id="inputDesk" class="w-full px-3 py-2 border rounded-lg" rows="3"></textarea>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2">Tanggal</label>
                        <input type="date" name="date" id="inputDate" class="w-full px-3 py-2 border rounded-lg" required>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2">Waktu</label>
                        <input type="time" name="time" id="inputTime" class="w-full px-3 py-2 border rounded-lg">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2">Status</label>
                        <select name="status" id="inputStatus" class="w-full px-3 py-2 border rounded-lg">
                            <option value="pending">Pending</option>
                            <option value="selesai">Selesai</option>
                        </select>
                    </div>
                    <div class="flex gap-2">
                        <button type="submit" class="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">Simpan</button>
                        <button type="button" onclick="closeModal()" class="flex-1 bg-gray-300 px-4 py-2 rounded-lg hover:bg-gray-400">Batal</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div id="confirmDeleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl p-6 max-w-sm w-full mx-4">
                ...
                <p class="mb-6">Apakah Anda yakin ingin menghapus acara ini?</p>
                <div class="flex gap-2">
                    <button id="confirmDeleteBtn" class="flex-1 bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">Hapus</button>
                    <button onclick="closeConfirmModal()" class="flex-1 bg-gray-300 px-4 py-2 rounded-lg hover:bg-gray-400">Batal</button>
                </div>
            </div>
        </div>
        <script>
            ...
            // ==================== MODAL FUNCTIONS ====================
            window.openModal = function(mode, activityId = null) {
                const modal = document.getElementById('modalAcara');
                const form = document.getElementById('formAcara');
                ...
                if (mode === 'add') {
                    ...
                    if (activity) {
                        ...
                        form.action = `/kalender/${activityId}`;
            ...
            document.getElementById('modalAcara')?.addEventListener('click', function(e) {
                if (e.target === this) {
                    closeModal();
                }
            });
